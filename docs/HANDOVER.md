...
- First gameplay foundation (deployed prototype, not yet a finished campaign level): optional `gameRole` on image layers with old untagged art treated as scenery; a crossing playtest starter; Edit/Play switch; read-only movement simulation with left/right/jump, gravity, solid and one-way platform collision, goal detection, fall reset, restart, and keyboard/touch buttons. `src/game.ts` is the pure simulation; `src/PlayMode.tsx` is the play UI. **Play scene currently lives inside the Manuscript sandbox solely as a shared-engine prototype.** The campaign mode, its curated levels, and its progression model are not yet implemented. Do not reframe a sandbox page as a campaign level.
...
1. Design the top-level separation between Manuscript sandbox and Game campaign, preserving the existing sandbox files and tools. Then design a separate campaign level entity and the fixed-scene/chosen-piece construction loop. The prototype is a scene test, **not** the first guided level. Do not make book page indices become level IDs or completion records.
2. Decide the first reusable active-object interactions and outcome-based construction validation with the user. A campaign level can have multiple valid solutions. The bridge and moat are examples rather than complete mechanics specifications.

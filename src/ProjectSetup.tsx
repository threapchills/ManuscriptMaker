...
export default function ProjectSetup({onCreate}:{onCreate:(options:ProjectSetupOptions)=>void}){
  ...
  return <div className="project-setup">
    ...
    {mode==='book'&&<div className="template-grid">{[{id:'bestiary',name:'The curious bestiary',note:'Creatures and illuminated lettering',art:'dragon'},{id:'botanical',name:'The secret garden',note:'A flowering first page',art:'blue-vine'},{id:'blank',name:'A blank folio',note:'Nothing but possibility',art:''},{id:'crossing',name:'Playable crossing',note:'A small movement and bridge playtest',art:'bridge-arch'}].map(item=><button key={item.id} className={template===item.id?'selected-template':''} aria-pressed={template===item.id} onClick={()=>{setTemplate(item.id);if(item.id==='crossing'){setWidth('1280');setHeight('720');setPageCount('1')}}}><span className={`template-preview ${item.id}`}>{item.art?<img src={`/ManuscriptMaker/assets/${item.art}.png`} alt=""/>:<Plus size={30}/>}</span><strong>{item.name}</strong><small>{item.note}</small></button>)}</div>}

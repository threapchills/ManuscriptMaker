...
import { SCENE_GROUPS, sceneConfig } from './sceneCatalog';
import type { SceneGroup } from './sceneCatalog';

type Collection = 'scene' | 'parts' | 'complete';
type Category = 'All' | 'Favorites' | AssetCategory | SceneGroup;

export default function ArtLibrary({ onAdd, onUpload, hidden, allowedIds }: { onAdd: (id: string) => void; onUpload: () => void; hidden: boolean; allowedIds?: string[] }) {
  const [collection, setCollection] = useState<Collection>('scene');
  ...
  const collectionAssets = ASSETS.filter(asset => (!allowedIds || allowedIds.includes(asset.id)) && (collection === 'scene' ? !!sceneConfig(asset.id) : (asset.kind === 'part') === (collection === 'parts'))).sort((a,b)=>allowedIds?allowedIds.indexOf(a.id)-allowedIds.indexOf(b.id):0);
  const categories: Category[] = ['All', ...(collection === 'scene' ? SCENE_GROUPS.filter(group => collectionAssets.some(asset => sceneConfig(asset.id)?.group === group)) : [...new Set(collectionAssets.map(asset => asset.category))]), 'Favorites'];
  const assets = collectionAssets.filter(asset =>
    (category === 'All' || (category === 'Favorites' ? favorites.includes(asset.id) : collection === 'scene' ? sceneConfig(asset.id)?.group === category : asset.category === category)) &&
    `${asset.name} ${asset.category} ${sceneConfig(asset.id)?.group||''} ${asset.tags.join(' ')}`.toLowerCase().includes(query.trim().toLowerCase())
  );
  ...
  return <div className="art-library" hidden={hidden}>
    <div className="library-intro">
      ...
      <p>{allowedIds ? 'Place these pieces to make a path. Your character must reach the signpost to turn the page.' : collection === 'scene' ? 'Choose a sky, build the ground, place a character, then press Play scene.' : collection === 'parts' ? 'A wall, a wing, a wild invention. Build it piece by piece.' : 'Complete creatures and curiosities to start a story.'}</p>
      <div className="collection-switch" role="group" aria-label="Artwork collection">
        <button aria-pressed={collection === 'scene'} onClick={() => { setCollection('scene'); setCategory('All'); }}>Build a scene</button>
    ...
    <div className="collection-meta"><span>{category === 'All' ? collection === 'scene' ? 'SCENE PIECES' : collection === 'parts' ? 'BUILDING PIECES' : 'COMPLETE ILLUSTRATIONS' : category.toUpperCase()}</span><span>{assets.length} pieces</span></div>
    <div className="asset-grid">{assets.map((asset, index) => <div className="asset-card" key={asset.id} style={{ animationDelay: `${Math.min(index, 8) * 30}ms` }}>
      <button className="asset-picture" title={`Add ${asset.name} · or drag onto the page`} onClick={() => onAdd(asset.id)} draggable onDragStart={event => { event.dataTransfer.setData('application/x-manuscript-asset', asset.id); event.dataTransfer.effectAllowed = 'copy'; }}>
        <img src={asset.src} alt={asset.name} loading={index > 5 ? 'lazy' : 'eager'} draggable={false}/><span className="asset-add"><Plus size={16}/></span>{collection==='scene'&&<span className="scene-tag">{sceneConfig(asset.id)?.group}</span>}
      </button>
